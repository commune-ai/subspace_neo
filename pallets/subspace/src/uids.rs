...
impl<T: Config> Pallet<T> { 
    ...
    pub fn replace_module( netuid: u16, uid_to_replace: u16, new_hotkey: &T::AccountId, block_number:u64 ) {

        log::debug!("replace_module( netuid: {:?} | uid_to_replace: {:?} | new_hotkey: {:?} ) ", netuid, uid_to_replace, new_hotkey );
    ...
    pub fn append_module( netuid: u16, new_hotkey: &T::AccountId, block_number:u64 ) {

        // 1. Get the next uid. This is always equal to subnetwork_n.
        let next_uid: u16 = Self::get_subnetwork_n( netuid );
        log::debug!("append_module( netuid: {:?} | next_uid: {:?} | new_hotkey: {:?} ) ", netuid, new_hotkey, next_uid );
    ...
    //
    pub fn get_hotkey_for_net_and_uid( netuid: u16, module_uid: u16) ->  Result<T::AccountId, DispatchError> {
    ...
    pub fn get_stake_for_uid_and_subnetwork( netuid: u16, module_uid: u16) -> u64 { 
        if Self::is_uid_exist_on_network( netuid, module_uid) {
            return Self::get_total_stake_for_hotkey( &Self::get_hotkey_for_net_and_uid( netuid, module_uid ).unwrap() ) 
        } else {
            return 0;
        }
    }
